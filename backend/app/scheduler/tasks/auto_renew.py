...
3. При ошибке: уведомляет пользователя, повторяет через 6 часов (до 2 попыток)
"""
...
async def process_auto_renewals() -> None:
    ...
    3. Если ошибка - уведомляем, повторяем при следующем запуске (до 2 попыток за 24 часа)
